...
//ReferenceError: Expected 'aVariableWithWrongName' not defined`
    },
    {
        ...
        "runAble": true,
        "example": `fn test(param: string, secondParam: string) {}
test("test");
//^
...
v GoodVariable: string;`
    ...
    },
    {
        ...
        "runAble": true,
        "example": `fn test(param: string, param: string) {}
//                       ^
...
v GoodEllie = ['e', 'l', 'l', 'i', 'e'];`
    ...
    },
    {
        ...
        "runAble": true,
        "example": `v testCloak : cloak(int, string) = (1, "test", 't');
//                                              ^
...
//TypeError: 'aVariable' is not a function`
    ...
    },
    {
        ...
        "runAble": false,
        "example": `import ./aFileThatBroken;
//             ^
//Cannot resolve './aFileThatBroken' module
...
//TypeError: 'aVariableNotExists' is not found in properties`
    },
    {
        ...
        "runAble": true,
        "example": `fn multipleArgs(*param: aDataType, secondParam: string) {}
//                                    ^
//SyntaxError: Cannot add a new parameter after multiple parameter`
    },
    {
        ...
        "runAble": true,
        "example": `v collective = {
    false: "test",
    //^
    //'bool' cannot be used as collective parameter
};
    `
    },
    {
...
//ReferenceError: Cannot define built-in types`
    },
    {
        ...
        "runAble": true,
        "example": `@parserInfo=1;
//          ^
//Parser messages value can only be string, but found 'int'
    `
    },
    {
        "code": "0x40",
...
v aInt = new Test().aVariableNotExists;
//                          ^
//'aVariableNotExists' is not found in 'Test' properties
    `
    },
    {
        "code": "0x42",
...
for (var i = 0; i < errors.length; i++) {
    ...
    if (!error.ignored) {
        ...
        h3.innerHTML = `#${error.code} - ${error.title}`;

        var h5 = document.createElement("h5");
        h5.innerHTML = error.detail;
        div.appendChild(h3);
        div.appendChild(h5);
        var codeDiv = document.createElement("div");
        codeDiv.id = error.code + "e";
        div.appendChild(codeDiv);
        document.getElementById("errors").appendChild(div);
        if (error.example) {
            let code = new Codev(error.code + "e");
            code.supplyCode(error.example);
        }
        light = !light;
    }
