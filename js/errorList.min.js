...
//ReferenceError: Expected 'string' found 'int'`
    ...
    co (exampleParam);
    //       ^
    //Targeted variable 'exampleParam' not found in scope
    ...
    },
    {
        "code": "0x04",
        "title": "SyntaxError",
        "detail": "Wrong escape sequence used in string",
        "example": `
v test = "\\e"
//         ^
//Unknown escape character '\e'
`,
    },
    {
        "code": "0x05",
...
v GoodVariable: string;`
    },
    {
        "code": "0x08",
        "title": "ReferenceError",
        "detail": "Expected a type that can be assigned while compiling",
        "runAble": true,
        "example": `@fileKeyTest = aRef;
//               ^            
//ReferenceError: Static type expected
`
    },
    {
        "code": "0x09",
...
v GoodEllie = ['e', 'l', 'l', 'i', 'e'];`
    ...
    },
    {
        ...
        "runAble": true,
        "example": `v testCloak : (int, string) = (1, "test", 't');
//                                              ^
//Fixed size exceeded: expected '2' elements, got '3' elements`
    },
    {
        "code": "0x19",
        "title": "TypeError",
        "detail": "Negative sized array is illegal",
        "example": `
v test : [int, -3];
//              ^
//Negative number is not allowed on 'array'`
    },
    {
        "code": "0x20",
        "title": "SyntaxError",
...
//SyntaxError: Reserved keyword 'class'`
    },
    {
        "code": "0x21",
        "title": "SyntaxError",
        "detail": "Appended element is cannot live in class body",
        "runAble": true,
        "example": `class Test {
    if true {}
    //  ^
    //Defined item cannot exist in class
}`
    },
    {
        "code": "0x22",
...
//                                     ^
//Cannot add a new parameter after multiple parameter`
    },
    {
        "code": "0x35",
        "title": "SyntaxError",
...
`
    },
    {
        "code": "0x43",
        "title": "ReferenceError",
        "detail": "Insufficient arguments supplied to generic definition",
        "runAble": true,
        "example": `class Test<T,E> {}
v def : Test<int>;
//       ^
//Target class requires '2' generic parameters, found '1' generic parameters
`
    },
    {
        "code": "0x44",
        "title": "ReferenceError",
        "detail": "Non-type supplied as a generic",
        "runAble": true,
        "example": `v test = 123;
v def : test;
//       ^
...
`
    },
    {
        "code": "0x45",
        "title": "ReferenceError",
        "detail": "Referenced type does not implement 'as' convertor",
        "runAble": true,
        "example": `class test {}
v def = new test() as int;
//           ^
//'test' is foreign type and does not implement 'as' convertor
`
    },
    {
        "code": "0x46",
        "title": "RuntimeRestriction",
        "detail": "Used syntax type or item is not supported in this environment",
        "runAble": true,
        "example": `class test {}
v def = new test() as int;
//                 ^
//As convertor is not supported in this environment
`
    },
    {
        "code": "0x47",
        "title": "ReferenceError",
        "detail": "Targeted type does not implement index queries",
        "runAble": true,
        "example": `class test {}
v def = new test()[0];
//                 ^
//'test' is foreign type and does not implement index queries
`
    },
    {
        "code": "0x48",
        "title": "ReferenceError",
        "detail": "Cannot query index to '$target' with '$token'",
        "runAble": true,
        "example": `v test = [1,2,3];
v def = test["a"];
//            ^
//String "a" is not a valid index of 'test'
`
    },
    {
        "code": "0x50",
        "title": "TypeError",
        "detail": "Cannot resolve non nullAble foreign type",
        "runAble": true,
        "example": `v test = 1;
v def = test!;
//          ^
//Cannot resolve non nullAble foreign type
`
    },
    {
        "code": "0x51",
        "title": "TypeError",
        "detail": "'$opType' not implemented in '$target' for '$value'",
        "runAble": true,
        "example": `class test {}
v def = new test() > 1;
//                 ^
//'Greater' not implemented in 'test' for 'int'
`
    },
    {
        "code": "0x42",
        "title": "SyntaxError",
        "detail": "$opType is not chainable",
        "runAble": true,
        "example": ` 123 < 123 < 123;
//SyntaxError: Comparison operator is not chainable
    `
    }
];
let light = !1;
for (var error, i = 0; i < errors.length; i++)
    if (error = errors[i], !error.ignored) {
        var a = document.createElement("a");
        a.innerHTML = error.code, a.href = `#${error.code}`, document.getElementById("btns").appendChild(a);
        var div = document.createElement("div");
        div.id = error.code, div.className = light ? "dark-section section" : "light-section section";
        var h3 = document.createElement("h3");
        h3.innerHTML = `#${error.code} - ${error.title}`;
        var h5 = document.createElement("h5");
        h5.innerHTML = error.detail, div.appendChild(h3), div.appendChild(h5);
        var codeDiv = document.createElement("div");
        if (codeDiv.id = error.code + "e", div.appendChild(codeDiv), document.getElementById("errors").appendChild(div), error.example) {
            let a = new Codev(error.code + "e");
            a.supplyCode(error.example)
        }
        light = !light
    }
